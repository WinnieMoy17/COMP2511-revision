...
public class DatabaseTest {
        ...
        public void SimpleTestPartC() {
                ...
                db.ingest(
                                ...
                                "StudentId|Tutorial|GroupSuffix|Assignment|Lab1|Lab2\n" +
                                // first row
                                                "z1234    |tue15a  |elderberry |35        |0   |2\n" +
                                                // first row
        ...
        public void SimpleTestPartD() {
                ...
                db.addDerivedColumn("TotalMark", Arrays.asList("Assignment", "Lab1", "Lab2"),
                                (cols) -> (int) cols.get("Lab1") + (int) cols.get("Lab2")
                                                + (int) cols.get("Assignment"));
        ...
        // this is a pretty long function so I wouldn't waste time reading through how
        // it works,
        // the errors/exceptions are detailed enough that they should help by itself.
        public static void checkQueryResults(String idField, Map<String, Map<String, Object>> expected,
                                ...
                                if (expected.containsKey(row.get(idField))) {
                                        ...
                                        if (!row.keySet().equals(expectedRow.keySet())) {
                                                throw new RuntimeException(
                                                                "There were extra unexpected columns... we expected the following columns (comma separated); ["
                                                                                +
                                                                                expectedRow.keySet().stream().collect(
                                                                                                Collectors.joining(","))
                                                                                +
                                                                                "] but we got ["
                                                                                + row.keySet().stream().collect(
                                                                                                Collectors.joining(","))
                                                                                + "]");
                                        }
                                        ...
                                        for (Map.Entry<String, Object> column : row.entrySet()) {
                                                // confirm values
                                                if (!Objects.equals(expectedRow.get(column.getKey()),
                                                                column.getValue())) {
                                                        throw new RuntimeException("Column " + column.getKey()
                                                                        + " did not match for record "
                                                                        + idField + " = " + row.get(idField) +
                                                                        " we were expecting the value "
                                                                        + column.getKey() + " = "
                                                                        + expectedRow.get(column.getKey()) +
                                                                        " but we got " + column.getKey() + " = "
                                                                        + column.getValue());
                                                }
                                        }
                                } else {
                                        throw new RuntimeException(
                                                        "Unexpected row, we weren't expecting a row where " + idField
                                                                        + " = " + row.get(idField));
                                }
                        } else {
                                throw new RuntimeException("We were expecting all rows to have a column called "
                                                + idField
                                                + " but it seems that the row containing these values did not; " +
                                                row.entrySet().stream().map(x -> x.getKey() + " = " + x.getValue())
                ...
                if (seenEntities.size() != expected.size()) {
                        throw new RuntimeException(
                                        "We were expecting " + expected.size() + " rows but we got "
                                                        + seenEntities.size() + " instead... "
                                                        +
                                                        "the rows that were missing had the following values for the column "
                                                        + idField + " [" +
